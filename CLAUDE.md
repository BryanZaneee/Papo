...
npm run test:game  # headless logic check for the runner game
...
- **`src/runner-game.js`** (~900 lines) — 3-lane pseudo-3D runner game (vanilla Canvas 2D) that gates the EPK. Player dodges/jumps obstacles and collects oranges. A clean run is ~12.5 s with no oranges, ~10 s grabbing most (`FINISH_DISTANCE` / `BASE_SPEED` / `SPEED_RAMP`); each orange is +1 and a short speed burst (`boost`, `BOOST_MULT`/`BOOST_DECAY`). Renders the world on the full-screen `#gameCanvas`; **all UI is DOM** (`.game-ui` panels + `.game-hud` in `index.html`, shown by `data-state` on `#gameOverlay`). Keyboard (arrows/WASD/space/enter) + pointer on the whole overlay (swipe ← → lanes, swipe ↑ or tap = jump, tap = start/retry). States: `title → playing → crashed | finished`, `endless` flag. **Loaded first and NOT deferred** (the other scripts are `defer`). Logic check: `npm run test:game` (headless, stubbed DOM/canvas).
  - **Two looks, one `THEMES` table** — `night` (site palette: stars, amber horizon, orange-lit shoulders) and `day` (sun, clouds, green grove). Picked by local hour (day 07:00–18:59); `?theme=day|night` overrides. `data-theme` on the overlay tints the DOM UI. Every draw reads `T.*`; geometry is shared.
  - **Obstacle colour encodes affordance, not object identity** — `pole` and `car` are dodge-only (tall, red-lit), `speaker` is jumpable (low, yellow band on top). Roadside orange trees are fogged (`fogAt()`) and pushed off the shoulder so scenery can never be confused with an obstacle. Don't reintroduce per-object art that breaks this.
  - **Rows are spaced by time, not metres** (`speed × GAP_SECONDS`), one obstacle per row, so the reaction window stays constant as speed climbs and two lanes are always safe. Oranges come in strings of three in a free lane, or arched over a speaker (`h` > 0 — only reachable mid-jump).
  - **All drawing is in CSS pixels via the module-level `W`/`H`, never `canvas.width`/`canvas.height`** — `resize()` scales the backing store by `devicePixelRatio` (capped at `MAX_DPR`) and pre-transforms the context. Viewport-sized gradients, the grove `ridges` and `stars` are cached in `buildStatic()` on resize.
  - **Finishing is not terminal**: `ENTER EPK` (`enterSite()`) or `KEEP RUNNING` (`goEndless()`, drops the finish line, speed climbs to `MAX_SPEED`). The crash panel offers `RETRY` and `ENTER EPK`; the title panel puts the skip link directly under `SPACE TO START`.
  - The rAF loop stops (`running = false`) once the EPK is revealed. `window.skipRunnerGame` is what `content.js` calls in `?preview=1`.
...
- **Typography**: Anton (hero name), Bebas Neue (game buttons/headings), Bricolage Grotesque (section headings), Archivo (body), Space Mono (labels/mono) — all from Google Fonts.
